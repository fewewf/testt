...
// 备用服务器列表
const РЕЗЕРВНІ_СЕРВЕРИ = [
    'ProxyIP.CA.CMLiussss.net:443'
];

// 超时设置 (毫秒)
const ЧАСОЧІКУВАННЯ_ПІДКЛЮЧЕННЯ = 5000; // 5秒
const МАКС_СПРОБ = 3;
...
async function обробкаВебСокету(запит, налаштування) {
    ...
    читабельнийВебСокет.pipeTo(new WritableStream({
        async write(фрагмент) {
            ...
            // Функція для підключення з таймаутом
            async function підключенняЗТаймаутом(адреса, порт, таймаут = ЧАСОЧІКУВАННЯ_ПІДКЛЮЧЕННЯ) {
                const контролер = new AbortController();
                const таймер = setTimeout(() => контролер.abort(), таймаут);
                
                try {
                    const сокет = await connect(
                        { hostname: адреса, port: порт }, 
                        { signal: контролер.signal, allowHalfOpen: true }
                    );
                    clearTimeout(таймер);
                    return сокет;
                } catch (помилка) {
            ...
            // Функція для спроби підключення з резервними серверами
            async function спробаПідключенняЗРезервом(початковийСервер, початковийПорт, дані) {
                // Список серверів для спроб (основний + резервні)
                const сервериДляСпроб = [
                    { адреса: початковийСервер, порт: початковийПорт },
                    ...РЕЗЕРВНІ_СЕРВЕРИ.map(сервер => {
                        const [адреса, порт] = сервер.includes(':') 
                            ? сервер.split(':') 
                            : [сервер, 443];
                        return { адреса, порт: parseInt(порт) || 443 };
                    })
                ];
                
                let останняПомилка = null;
                
                for (let i = 0; i < сервериДляСпроб.length; i++) {
                    const { адреса, порт } = сервериДляСпроб[i];
                    try {
                        const сокет = await підключенняЗТаймаутом(адреса, порт);
                        const записувач = сокет.writable.getWriter();
                        await записувач.write(дані);
                        записувач.releaseLock();
                        return сокет;
                    } catch (помилка) {
                        останняПомилка = помилка;
                        console.log(`Спроба ${i + 1} не вдалася для ${адреса}:${порт}:`, помилка.message);
                        // Продовжуємо з наступним сервером
                    }
                }
                
                throw останняПомилка || new Error('Всі сервери недоступні');
            }
            
            try {
                // Використовуємо нову функцію для підключення з резервом
                віддаленийСокет = await спробаПідключенняЗРезервом(
                    результат.адресаВіддаленого, 
                    результат.портВіддаленого, 
                    сиріДаніКлієнта
                );
                
                передачаДанихДоВебСокету(віддаленийСокет, сервернийВебСокет, відповідьVless, {
